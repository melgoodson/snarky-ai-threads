...
serve(async (req) => {
  ...
  try {
    ...
    const enhancedPrompt = `Create a standalone artwork design. ${prompt}. IMPORTANT: Generate ONLY the flat design/artwork itself on a solid white background. Do NOT generate any checkerboard patterns, gray-and-white grid overlays, transparency grids, or transparency representations. The background MUST be a single solid color (white). Do NOT show the design on any product like a shirt, hoodie, mug, or any merchandise. No product mockups. Just the pure artwork/illustration/graphic by itself, high resolution, vibrant colors, clean edges, print-ready.`;
